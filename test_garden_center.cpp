...
void demonstrateTemplateMethod() {
    ...
    try {
        ...
        // C++11 version: use std::unique_ptr constructor instead of make_unique
        reportGen.registerReport("inventory", std::unique_ptr<Report>(new InventoryReport(nullptr)));
        
        // Use current time for sales report
        std::time_t now = std::time(nullptr);
        reportGen.registerReport("sales", std::unique_ptr<Report>(new SalesReport(nullptr, now, now)));
        ...
        std::cout << std::string(40, '-') << "\n";
        std::unique_ptr<InventoryReport> inventoryReportObj(new InventoryReport(nullptr));
...
void demonstrateIntegratedSystem() {
    ...
    try {
        ...
        // C++11 version: use std::unique_ptr constructor
        dailyReportGen.registerReport("inventory", std::unique_ptr<Report>(new InventoryReport(nullptr)));
        
        std::time_t now = std::time(nullptr);
        dailyReportGen.registerReport("sales", std::unique_ptr<Report>(new SalesReport(nullptr, now, now)));
